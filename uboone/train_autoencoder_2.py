...
AutoEncoder, Encoder=autoencoder.build(input_img)
...
for step in xrange(cfg.ITERATIONS):
  ...
  resize_img=np.zeros((cfg.BATCH_SIZE,1024,1024,1))
  idx=0
  for each in img:
    result=np.zeros((1024,1024,1))
    result[:1008,:,:1]=each[:,:1024,:1]
    resize_img[idx]=result
    idx+=1

  input_img=np.zeros((cfg.BATCH_SIZE,1024,1024,1))
  idx=0

  for each in resize_img:
    each=each.reshape(1024**2,)
    each=each/sum(each)
    input_img[idx]=each.reshape(1024,1024,1)
    idx+=1
      
  here=cfg.BATCH_SIZE/10
  ...
  for x in xrange(9):
    ...
    ax.imshow(uboone_train[x].reshape(1024,1024))
  ...
  parallel_model.save_weights('trainweights/weights_%i.h5'%((step+1)*epochs))
