...
#parallel_model  =  multi_gpu_model(AutoEncoder, gpus = 5, cpu_relocation=True)
#parallel_model.compile(optimizer = 'Adam', loss = 'binary_crossentropy')
parallel_model=ParallelModel(AutoEncoder, gpu_count=GPU_COUNT)
...
#for step in xrange(cfg.ITERATIONS):
  ...
  #remove empty image in training data
  train_keep=np.zeros(uboone_train.shape[0], dtype=bool)
  for x in xrange(uboone_train.shape[0]):
    if np.sum(uboone_train[x]>0) > 100 : train_keep[x]=True
  uboone_train=uboone_train[train_keep]

  if (uboone_train.shape[0]<GPU_COUNT): continue
  else:
    slice_here=uboone_train.shape[0] % GPU_COUNT
    print uboone_train.shape[0], GPU_COUNT
    print 'slice in train is ', slice_here
    print uboone_train[slice_here:].shape
    uboone_train=uboone_train[slice_here:]
  print 'filtered train shape', uboone_train.shape

  #remove empty image in test data
  test_keep=np.zeros(uboone_test.shape[0], dtype=bool)
  for x in xrange(uboone_test.shape[0]):
    if np.sum(uboone_test[x]>0) > 100 : test_keep[x]=True
  uboone_test=uboone_test[test_keep]
  
  if (uboone_test.shape[0]<GPU_COUNT): continue
  else:
    slice_here=uboone_test.shape[0] % GPU_COUNT
    print uboone_test.shape[0], GPU_COUNT
    print 'slice in test is ', slice_here
    print uboone_test[slice_here:].shape
    uboone_test=uboone_test[slice_here:]
  print 'filtered test shape',uboone_test.shape
  ...
  #epochs=int(uboone_train.shape[0]/5)
  epochs=15
  ...
  #initial_epoch+=uboone_train.shape[0]
  trained_image_counter+=uboone_train.shape[0]
  if(step % 10==0): 
    parallel_model.save_weights('trainweights/weights_%i_%i.h5'%((step, trained_image_counter)))
    #parallel_model.save('trainweights/weights_%i.h5'%((step)))
